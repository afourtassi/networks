...
######################################################################################################################
get_lang_item_data <- function(lang, lang_form = "WG", lex_class = "nouns") {
...
######################################################################################################################
# get only "useful" admin data (where comprehension is not NA)
get_lang_admin_data <- function(lang, lang_form = "WG") {
  #get the kids' data_id from every age
  admin_data <- get_administration_data() %>%
    filter(form == lang_form, !is.na(comprehension), language == lang) %>%
    select(data_id, age) %>%
    arrange(age)
...
get_lang_instr_data <- function(lang, lang_form = "WG") {
...
######################################################################################################################
make_aoa_dataframe <- function(lang, lang_form = "WG", lex_class = "nouns") {
...
# 
# ######################################################################################################################
# # See how many kids we have in every month
...
#   ages<- nkids_by_month$month
#   
#   for (month in months) {
#     rem_item <- item_data %>% filter(month == "no")
#     current_month_id <- admin_data %>% filter(age == month)
#     current_instr <- instr_data %>% 
#       filter(data_id %in% current_month_id$data_id)
#     for (w in rem_item$item) {
#       proportion <- sum(current_instr$item == w) / nkids_by_month$n[which(months==month)]
#       if (proportion >= 0.5) {
#         item_data$month[which(item_data$item == w)] = month
#       }
#     }
#   }
#   word_aoa<- item_data %>% filter(month != "no")
#   return(word_aoa)
# }
# 
# ######################################################################################################################
...
#     curr_df <- data.frame(corr_month, rem_item, rem_lemma, rem_def)
#     df <- rbind(df, curr_df)
#   }  
#     df <- df %>% rename(uni_lemma = rem_lemma, definition=rem_def, item=rem_item)%>%
#       left_join(word_aoa %>% select(item, month)) %>%
