...
pair_data <- function(lang1){
  ...
  data_lang1 <- lang1 %>%
    ...
    select(uni_lemma, PAC_phono) %>%
    group_by(uni_lemma) %>%
    summarise(deg_lang1 = mean(PAC_phono))
  ...
  data_lang2 <- lang2 %>%
    ...
    select(uni_lemma, PAC_phono) %>%
    group_by(uni_lemma) %>%
    summarise(deg_lang2 = mean(PAC_phono_t2))
  
  both <- data_lang1 %>%
    left_join(data_lang2)
  
  mycorr <- cor.test(both$deg_lang1, both$deg_lang2)
...
#feather::write_feather(cor_data, "saved_data/cor_data.feather")
cor_data <- feather::read_feather("saved_data/cor_data.feather") 
...
#phono_pairs_wide <- phono_pairs_dist %>%
 # spread(language, dist)
#Pairwise comparision: each time, take the overlap and do the test

phono_pairs_produce <- phono_pairs_dist %>%
  filter(measure == "produces")


data_by_lang <- phono_pairs_produce %>% 
  split(.$language)
...
pair_data <- function(lang1){
  ...
  lang1_unilemmas <- unique(lang1$item.uni_lemma)
  lang2_unilemmas <- unique(lang2$item.uni_lemma)
  
  uni_intersect <-  intersect(lang1_unilemmas, lang2_unilemmas)
  
  data_lang1 <- lang1 %>%
    filter(item.uni_lemma %in% uni_intersect,
           pair.uni_lemma %in% uni_intersect) %>%
    select(item.uni_lemma, pair.uni_lemma, link2) %>%
    rename(link2_lang1 = link2)
  
  data_lang2 <- lang2 %>%
    filter(item.uni_lemma %in% uni_intersect,
           pair.uni_lemma %in% uni_intersect) %>%
    select(item.uni_lemma, pair.uni_lemma, link2) %>%
    rename(link2_lang2 = link2)
  
  data_both <- data_lang1 %>%
    left_join(data_lang2) 
  
  #data_mat <- acast(data_both, item.uni_lemma~pair.uni_lemma, value.var="link2_lang1")
  
  #This too costly computationnally speaking, will try the one-dimentional analysis instead, it should be enough
...
cor_data <- map(growth_by_meas_lang, pair_data) %>%
  bind_rows() %>%
  filter(language1!=language2) %>%
  mutate(data="languages")
...
Here I will do the analysis for the power law (less computation-costly and more relant to the rest of the analyses)
