...
#I should here define the set of words that will be used in the network construction
#Here I chose to use the full data for the phono netwotk and filter later for the unilemmas only, thus the utility will be more precise for the resulting subset 
...
#Word frequency: we use frequency values derived from Childes (from Mika e al. 2016) 
...
#The following analyses predict try to predict the items' AoA. I keep duplicated values of the predictors for different items (when they have the same defintions and/or unilemma) since I consider that this is the actual (albeit braod) information brough by the precitors
...
Use igraph to derive small world properties

For each netwrok, compute:
-Clustering coefficient 
-Shortest path length 
what we want: Compared to an equivalent random network, we want a balance between local processing (high clustering coefficient) and global integration (small short path length)
...
data_growth_pred <- data_growth_net %>%
  ...
  filter(!is.na(PAC_assoc)) %>% # keep unilemma whcih intersect with free association data
  filter(!is.na(PAC_phono_t2)) # In case a word is not trnascribed phonologically (there should not be any)
